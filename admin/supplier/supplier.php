...
if ($search !== '') {
    $keyword = '%' . $search . '%';
    $supplierIdSearch = intval(preg_replace('/\D+/', '', $search));

    $where[] = "(
        supplier_name LIKE ?
        OR phone_number LIKE ?
        OR address LIKE ?
        OR CAST(supplier_id AS CHAR) LIKE ?
        OR supplier_id = ?
    )";
...
/* Get suppliers */
$listSql = "
    SELECT supplier_id, supplier_name, phone_number, address
    FROM tb_supplier
    $whereSql
...
<head>
    <meta charset="UTF-8">
    <title>ຈັດການຜູ້ສະໜອງ</title>

    <link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/dashboard.css?v=<?= time(); ?>">
    <link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/supplier.css?v=<?= time(); ?>">
</head>
<body>
            ...
            <form method="GET" class="filter-box">
                ...
                <div class="filter-group">
                    ...
                    <input
                        type="text"
                        name="search"
                        placeholder="ຄົ້ນຫາລະຫັດ, ຊື່ຜູ້ສະໜອງ, ເບີໂທ, ທີ່ຢູ່..."
                        value="<?= h($search); ?>"
                    >
                </div>
                ...
                <div class="filter-actions">
                    ...
                    <a href="<?= BASE_URL ?>/admin/supplier/supplier.php" class="btn-reset">
                        ລ້າງ
                    </a>
                </div>
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        ...
                        <?php while ($row = $result->fetch_assoc()): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?= "SUP-" . str_pad($row['supplier_id'], 4, "0", STR_PAD_LEFT); ?>
                                </td>
                                ...
                                <td>
                                    ...
                                    <a
                                        class="btn-edit"
                                        href="edit_supplier.php?id=<?= intval($row['supplier_id']); ?>"
                                    >
                                        ແກ້ໄຂ
                                    </a>
                                    ...
                                    <button
                                        type="button"
                                        class="btn-delete"
                                        onclick="openDeleteModal(<?= intval($row['supplier_id']); ?>)"
                                    >
                                        ລຶບ
                                    </button>
...
function openDeleteModal(id)
{
    ...
    document.getElementById("deleteLink").href =
        "<?= BASE_URL ?>/actions/supplier/delete_supplier_action.php?id=" + id;
}
