...
if ($search !== '') {
    $keyword = '%' . $search . '%';
    $unitIdSearch = intval(preg_replace('/\D+/', '', $search));

    $where[] = "(
        unit_name LIKE ?
        OR CAST(unit_id AS CHAR) LIKE ?
        OR unit_id = ?
    )";
...
<div class="dashboard-layout">
    ...
    <main class="main-content">
        ...
        <div class="page-header">
            <div>
                <h1>ຈັດການຫົວໜ່ວຍ</h1>
                <p>ລາຍຊື່ຫົວໜ່ວຍທັງໝົດ</p>
            </div>

            <a href="add_unit.php" class="btn-primary">
                + ເພີ່ມຫົວໜ່ວຍ
            </a>
        </div>
            ...
            <form method="GET" class="filter-box">
                ...
                <div class="filter-group">
                    ...
                    <input
                        type="text"
                        name="search"
                        placeholder="ຄົ້ນຫາລະຫັດ ຫຼື ຊື່ຫົວໜ່ວຍ..."
                        value="<?= h($search); ?>"
                    >
                </div>
                ...
                <div class="filter-actions">
                    ...
                    <a href="<?= BASE_URL ?>/admin/unit/unit.php" class="btn-reset">
                        ລ້າງ
                    </a>
                </div>
                ...
                <tbody>
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                <td>
                                    <?= "UNT-" . str_pad($row['unit_id'], 4, "0", STR_PAD_LEFT); ?>
                                </td>
                                ...
                                <td>
                                    <a
                                        class="btn-edit"
                                        href="edit_unit.php?id=<?= intval($row['unit_id']); ?>"
                                    >
                                        ແກ້ໄຂ
                                    </a>
                                    ...
                                    <button
                                        type="button"
                                        class="btn-delete"
                                        onclick="openDeleteModal(<?= intval($row['unit_id']); ?>)"
                                    >
                                        ລຶບ
                                    </button>
                                </td>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal-box">
        ...
        <p>ທ່ານຕ້ອງການລຶບຫົວໜ່ວຍນີ້ບໍ?</p>
...
function openDeleteModal(id)
{
    ...
    document.getElementById("deleteLink").href =
        "<?= BASE_URL ?>/actions/unit/delete_unit_action.php?id=" + id;
}
